...
import example.wxx.com.baselibrary.http.IHttpEngine;
import okhttp3.Call;
        ...
        mOkHttpClient.newCall(request).enqueue(
                new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ...
                        Log.e(TAG, jointUrl + "-->Post返回结果：" + result);
...
//                每次获取到的数据，都对比上一次的内容
                if (isCache) {
//                从数据库中取出缓存
                    String cacheDataResultJson = CacheUtil.getCacheResultJson(finalUrl);
                    if (!TextUtils.isEmpty(cacheDataResultJson)) {
//                        对比
                        if (resultJson.equals(cacheDataResultJson)) {
